...
os.chdir('/home/user/Schreibtisch/git_env_new/ClearMap2-Athena')
...
if __name__ == '__main__':
  ...
  from ClearMap.Environment import *  #analysis:ignore Elastix
  
  #directories and files
#  directory = '/media/athena-admin/FastSSD/AnnaMaria_HuTestFiles'
#  directory = '/media/athena-admin/FastSSD/Peter_HuTestFiles'
  directory = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647'
  expression_raw      = '1.2.CX_P_[<Y,2> x <X,2>]_640_nm_F76_647SG_LP_4x.npy'
  expression_arteries = '1.2.CX_P_[<Y,2> x <X,2>]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x.npy'
  ...
  test_file = '/media/athena-admin/FastSSD/AnnaMaria_HuTestFiles/1.3.KH_hFTAA_SMA_Cy3_Pdxl-647_[03 x 02]_640_nm_F76_647SG_LP_4x_Left.npy'
  test_file = '/media/athena-admin/FastSSD/Peter_HuTestFiles/1.3.KH_hFTAA_SMA_Cy3_Pdxl-647_[03 x 03]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left.npy'
  test_file='/media/athena-admin/FastSSD/Peter_HuTestFiles/1.3.KH_hFTAA_SMA_Cy3_Pdxl-647_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left.npy'
  test_file = '/media/athena-admin/FastSSD/Peter_HuTestFiles/1.3.KH_hFTAA_SMA_Cy3_Pdxl-647_[02 x 03]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left.npy'
  test_file= '/media/user/Daten 1/HuTestFiles/1.3.KH_hFTAA_SMA_Cy3_Pdxl-647_[03 x 02]_640_nm_F76_647SG_LP_4x_Left.npy'
  test_file = '/media/user/Daten 1/HuTestFiles/1.3.KH_hFTAA_SMA_Cy3_Pdxl-647_[03 x 02]_640_nm_F76_647SG_LP_4x_Left.npy'
  test_fileX = '/media/user/Daten 1/HuTestFiles/binary.npy'
#  p3d.plot(test_file,min_max = [0, 800]) 
  p3d.plot(test_file) 


    p3d.plot(expression_raw) 
    p3d.plot(expression_arteries) 

  
  #%%############################################################################
  ### Stitching
  ###############################################################################
  #2500 um / 1.506 um/pixel = 1660 pixel
  #2048 pixel - 1660 pixel = 388 pixel
  
  
  
  
  #%% Rigid z-alignment    
  # background: number of pixels (1) that are above the value (2)  
          
  layout = stw.WobblyLayout(expression=ws.filename('raw'), tile_axes=['X','Y'], overlaps=(415, 415));  
  
#  st.align_layout_rigid_mip(layout, depth=[415, 415, None], max_shifts=[(-50,50),(-50,50),(-80,80)],
#                            ranges = [None,None,None], background=(150, 100), clip=25000, 
#                            processes=None, verbose=True)
  st.align_layout_rigid_mip(layout, depth=[415, 415, None], max_shifts=[(-80,80),(-80,80),(-80,80)],
                            ranges = [None,None,None], background=(100, 120), clip=25000, 
                            processes=None, verbose=True)
  ...
  f.write('Rigid z-alignment done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  ...
  # The third element of "axis_range" (here: "3") indicates that every 3rd slice will be wobbly-aligned
  # I assume that the rest of alignment of slices will be interpolated
  #
  # 
  #
  # validate_slice --> valid_range=(157,20000), size=1500
  # means that there must be >1500 pixels in the slice with values >157 and <20000, otherwise alignment is not attempted
  #
  # 
  
  layout = st.load_layout(ws.filename('layout', postfix='aligned_axis'))
  
#   axis_range=(None, None, None)
#   max_shifts=[(-30,30),(-15,15),(0,0)]
#   axis_mip=None
#   validate=dict(method='foreground', valid_range=(157, None), size=None)
#   prepare =dict(method='normalization', clip=None, normalize=True)
#   validate_slice=dict(method='foreground', valid_range=(157,20000), size=1500)
#   prepare_slice =None
#   find_shifts=dict(method='tracing', cutoff=3*np.sqrt(2)) . cutoff is an arbitrary threshold to cut off the distance matrix for pairing (tracking method)
#   processes=None
#   verbose=True
  
  processes_used = 9
  
  stw.align_layout(layout, axis_range=(None, None, None), max_shifts=[(-20,20),(-20,20),(0,0)], axis_mip=None,
                   validate=dict(method='foreground', valid_range=(125, None), size=None),
                   prepare =dict(method='normalization', clip=None, normalize=True),
                   validate_slice=dict(method='foreground', valid_range=(125,15000), size=1500),
                   prepare_slice =None,
                   find_shifts=dict(method='tracing', cutoff=3*np.sqrt(2)),
                   processes=processes_used, verbose=True)
  ...
  # a.shifts[1600:1700]
  # a.qualities[1600:1700]
  # a.status[1600:1700]
  
  
  f.write('Wobly alignment done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  ...
  stw.place_layout(layout, min_quality = -np.inf, 
                   method = 'optimization', 
                   smooth = dict(method = 'window', window = 'hamming', window_length = 100, binary = None), 
                   smooth_optimized = dict(method = 'window', window = 'bartlett', window_length = 20, binary = 10),                               
  ...
  f.write('Wobly stitching done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  
#  X = layout.alignments[0]
#  plt.figure; plt.plot(X.shifts)
#  planes are shifted by the change in z direction (X.displacement[2])
...
#  layout.replace_source_location(expression_raw, '1.2.CX_P_[<Y,2> x <X,2>]_640_nm_F76_647SG_LP_4x_flatfield.npy', method='expression')
  
  stw.stitch_layout(layout, sink = ws.filename('stitched', postfix='arteries'), method = 'interpolation', processes='serial', verbose=True)
  
  
  f.write('Wobbly stitching arteries done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  
  p3d.plot(ws.filename('stitched'),min_max = [130, 330])
 ...
 # resample_parameter = {
 #     "source_resolution" : (1.506,1.506,1),
...
#  source = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647/1.2.CX_P_[03 x 02]_640_nm_F76_647SG_LP_4x.npy'
  sink   = ws.filename('binary');

  
  binarization_parameter = vasc.default_binarization_parameter.copy();
  binarization_parameter['equalize']['selem']  = (500,500,5)
  binarization_parameter['equalize']['spacing']  = (100,100,5)
  binarization_parameter['adaptive']['selem']  = (500,500,5)
  binarization_parameter['adaptive']['spacing']  = (100,100,5)
  binarization_parameter['clip']['clip_range'] = (140, 2500)
  ...
  p3d.plot('/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647/binary_postprocessed.npy') 
  
  p3d.plot('/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/binary_old/binary_postprocessed.npy')
  ...
  f.write('Smoothing and filling done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  ...
  #%% Binarization - arteries  TESTESTESTESTEST ONLY
  

  source = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left.npy'
  
  
  sink1 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink1.npy'
  sink2 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink2.npy'
  sink3 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink3.npy'
  sink4 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink4.npy'
  sink5 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink5.npy'
  sink6 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink6.npy'
  sink7 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink7.npy'
  sink8 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink8.npy'
  
  binarization_parameter = vasc.default_binarization_parameter.copy();
  binarization_parameter['clip']['clip_range'] = (130, 1000)
  binarization_parameter['deconvolve']['threshold'] = 2000 
  binarization_parameter['median']['selem'] = (3, 3, 3)
  #binarization_parameter['equalize'] =  None
  #binarization_parameter['vesselize'] = None;
  
  #equalize': {'percentile': (0.4, 0.975),
  #'selem': (200, 200, 5),
  #'spacing': (50, 50, 5),
  #'interpolate': 1,
  #'threshold': 1.1,
  #'save': False},
  #'adaptive': {'selem': (250, 250, 3),
  #'spacing': (50, 50, 3),
  #'interpolate': 1,
  #'save': False},
  #'vesselize': {'background': {'selem': ('disk', (30, 30, 1)),
  #'percentile': 0.5},
  #'tubeness': {'sigma': 1.0, 'gamma12': 0.0},
  #'threshold': 120,
  #'save': False},
 ...
 processing_parameter.update(processes = None,
  ...
  binarization_parameter['equalize']['selem'] = (1000,1000,5)
  binarization_parameter['equalize']['spacing'] = (250,250,5)
  
  binarization_parameter['clip']['clip_range'] = (130, 1000)
  vasc.binarize(source, sink4, binarization_parameter=binarization_parameter, processing_parameter=processing_parameter);
  binarization_parameter['clip']['clip_range'] = (140, 1000)
  vasc.binarize(source, sink5, binarization_parameter=binarization_parameter, processing_parameter=processing_parameter);
  binarization_parameter['equalize']['selem'] = (500,500,5)
  binarization_parameter['equalize']['spacing'] = (100,100,5)
  binarization_parameter['clip']['clip_range'] = (140, 1000)
  vasc.binarize(source, sink6, binarization_parameter=binarization_parameter, processing_parameter=processing_parameter);
                   
  binarization_parameter['equalize'] = None
  binarization_parameter['clip']['clip_range'] = (140, 1000)
  vasc.binarize(source, sink7, binarization_parameter=binarization_parameter, processing_parameter=processing_parameter);
      
  binarization_parameter = vasc.default_binarization_parameter.copy();
  binarization_parameter['deconvolve']['threshold'] = 2000 
  binarization_parameter['median']['selem'] = (3, 3, 3)
  binarization_parameter['equalize']['selem'] = (1500,1500,5)
  binarization_parameter['equalize']['spacing'] = (250,250,5)
  binarization_parameter['adaptive']['selem'] = (1500,1500,5)
  binarization_parameter['adaptive']['spacing'] = (250,250,5)
  binarization_parameter['clip']['clip_range'] = (140, 1000)
  vasc.binarize(source, sink8, binarization_parameter=binarization_parameter, processing_parameter=processing_parameter);
                   

  sink9 = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink9.npy'
  binarization_parameter = vasc.default_binarization_parameter.copy();
  binarization_parameter['deconvolve']['threshold'] = 2000 
  binarization_parameter['median']['selem'] = (3, 3, 3)
  binarization_parameter['equalize']['selem'] = (1500,1500,5)
  ...
  binarization_parameter['adaptive']['spacing'] = (250,250,5)
  binarization_parameter['clip']['clip_range'] = (130, 1000)
  vasc.binarize(source, sink9, binarization_parameter=binarization_parameter, processing_parameter=processing_parameter);

  p3d.plot([source, sink9])
  
  source = sink9;
  sink   = sink9[:-4]+'_postprocessed.npy';
  ...
  f.write('Binarization - arteries done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  ...
  f.write('Smoothing and filling - arteries done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  ...
  source = '/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647/binary_postprocessed.npy'
  ...
  vf.fill_vessels(source, sink, resample=1, threshold=0.75, cuda=None, processing_parameter=processing_parameter, verbose=True)
  
  f.write('Vessel fillingdone\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')

  p3d.plot([source, sink]);
  p3d.plot([ws.filename('stitched'),ws.filename('binary")])
  
  #%% Vessel filling - arteries
                   
  source = ws.filename('binary', postfix='arteries_postprocessed');
  source = "/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647_tiles/1.2.CX_P_[03 x 02]_561_nm_F75_562SG_RAZOR_EDGE_LP_4x_Left_binarized_sink9_postprocessed.npy"
  ...
  vf.fill_vessels(source, sink, resample=15, threshold=0.75, cuda=None, processing_parameter=processing_parameter, verbose=True)
  
  f.write('Vessel filling - arteries done\n')
  f.write(str(datetime.datetime.now()))
  f.write('\n')
  
  p3d.plot([source, sink]);
  p3d.plot("/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647/binary_arteries_filled.npy")
  p3d.plot("/media/user/Daten1/ADf_1.2.CX_P_hFTAA_SMA-Cy3_Pdxl-647/binary_filled.npy")
  p3d.plot([ws.filename('stitched', postfix='arteries'),ws.filename('binary', postfix='arteries')])
  ...
  binary   = ws.filename('binary', postfix='arteries_filled');
  skeleton = ws.filename('skeleton', postfix='arteries')                 
  ...
  graph_raw = gp.graph_from_skeleton(ws.filename('skeleton', postfix='arteries'), verbose=True)
  graph_raw.save(ws.filename('graph', postfix='arteries'))
  ...
  p3d.plot_graph_line(graph_cleaned)
  ...
  #graph_reduced = grp.load(ws.filename('graph', postfix='reduced'));
    ...
    p3d.plot_graph_line(graph_reduced)
